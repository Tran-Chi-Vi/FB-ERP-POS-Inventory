...
export const CustomerQrPage: React.FC<CustomerQrPageProps> = ({ activeTab }) => {
  ...
  const products = [
    { id: '1', name: 'Cà Phê Sữa Đá', price: 29000, category: 'Cà Phê', status: 'Bán Chạy' },
    { id: '2', name: 'Bạc Xỉu Đá', price: 32000, category: 'Cà Phê', status: 'Còn Hàng' },
    { id: '3', name: 'Trà Đào Cam Sả', price: 39000, category: 'Trà Sữa', status: 'Còn Hàng' },
    { id: '4', name: 'Trà Sữa Ô Long', price: 42000, category: 'Trà Sữa', status: 'Bán Chạy' },
  ];
  ...
  return (
    <div>
      ...
      {/* 2. TIỆN ÍCH YÊU CẦU TẠI BÀN */}
      {activeTab === 'customer-service' && (
        <div className="card">
          <h2>Tiện Ích Yêu Cầu Phục Vụ Tại Bàn</h2>
          <p style={{ color: '#94a3b8', marginTop: '0.25rem' }}>Gửi thông báo tức thì tới đồng hồ thông minh / thiết bị của nhân viên phục vụ ca trực.</p>
          <div style={{ display: 'flex', gap: '1rem', marginTop: '1.25rem', flexWrap: 'wrap' }}>
            <button className="btn-primary" style={{ width: 'auto', padding: '0.75rem 1.5rem' }} onClick={() => triggerServiceCall('Gọi Nhân Viên')}>
              Gọi Phục Vụ Tại Bàn
            </button>
            <button style={{ padding: '0.75rem 1.5rem', background: '#334155', color: '#fff', border: 'none', borderRadius: '0.5rem', fontWeight: 'bold', cursor: 'pointer' }} onClick={() => triggerServiceCall('Xin Thêm Đá / Nước')}>
              Xin Thêm Đá / Nước Lọc
            </button>
            <button style={{ padding: '0.75rem 1.5rem', background: '#38bdf8', color: '#fff', border: 'none', borderRadius: '0.5rem', fontWeight: 'bold', cursor: 'pointer' }} onClick={() => triggerServiceCall('Yêu Cầu Thanh Toán')}>
              Yêu Cầu Thanh Toán Quầy
            </button>
          </div>
        </div>
      )}

      {/* 3. VÍ TÍCH ĐIỂM LOYALTY */}
      {activeTab === 'customer-loyalty' && (
        <div className="card">
          <h2>Ví Tích Điểm & Thẻ Thành Viên Virtual Loyalty</h2>
          <div style={{ padding: '1.25rem', background: '#0f172a', borderRadius: '0.5rem', marginTop: '1rem' }}>
            <h3 style={{ color: '#f59e0b' }}>Thẻ Thành Viên Hạng Gold</h3>
            <p style={{ color: '#94a3b8', marginTop: '0.25rem' }}>Tích lũy 1.250 điểm | Tiến trình thăng hạng Diamond: 75%</p>
          </div>
        </div>
      )}
    </div>
